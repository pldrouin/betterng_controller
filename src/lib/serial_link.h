...
#define BUILD_THREE_BYTES_CMD(cmd, ID, value1, value2) ({(cmd)->id=ID; (cmd)->byte1=value1; (cmd)->byte1=value2; calc_check_three_bytes(cmd);})
...
#define send_recv_three_bytes_cmd(sl_dev, ID, value1, resp_ID, value2, req_resp) ({BUILD_THREE_BYTES_CMD(&(req_resp)->req, ID, value1, value2); (req_resp)->resp.id=resp_ID; _sl_write_cmd(sl_dev, &(req_resp)->req, 4, &(req_resp)->resp);})

enum {SL_READ_OUT_OF_SYNC=1, SL_WRITE_OUT_OF_SYNC=2};
...
typedef struct
{
    ...
    pthread_cond_t status_clear_cond;
    pthread_mutex_t status_clear_cond_lock;
} sl_device;
